...
## C. DEPENDENCY UPGRADE (19 high-severity vulns) — plan: DEPS-UPGRADE-PLAN.md — **SHIPPED 2026-08-23 (26→0)**
...
- Wave3 (4f566dc) Clerk patch-line: `@clerk/backend@1.14.1→1.34.0` — `13→13` (dedupe).
- Wave4 (0bfe585) LHCI removal: `npm remove @lhci/cli` — `13→3` (only `next`/`postcss`/`sharp`).
- Wave5 (23736ee) overrides: `postcss@8.5.26` + `sharp@0.35.0` (`libvips 1.2.4→1.3.0`) — **3→0**, `next@15.5.23` retained (no major), plus `debug` oracles `404` in production (`/api/debug/transcription-test` + `/api/billing/debug`), `.env` `600`. `CI #555` green, `136/1139`.

Status: SHIPPED. No vulns remain. Ship order: before broad launch — done.
