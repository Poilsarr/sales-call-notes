...
| —   | 2026-08-16 | fix(upload): slow-path (>4MB) blob uploads rejected by trust guard | Root cause: @vercel/blob's canonical URL includes the access segment — `constructBlobUrl` builds `https://{storeId}.{access}.blob.vercel-storage.com/{pathname}` (SDK chunk-CIIQSN42.js), and with `access: 'private'` the presigned PUT response `url` (used by the client at record/page.tsx:307) is `store_x.private.blob.vercel-storage.com`. `isTrustedBlobUrl` only accepted the bare `{storeId}.blob.vercel-storage.com`, so `/api/analyze` rejected every slow-path blobUrl with "Invalid blobUrl: must point to this store" at 20%. Fix: guard accepts bare + private + public access-qualified hostnames (still scoped to our store; other stores still rejected); `/api/upload-url` constructs `blobUrl` with the `.private` segment; 3 new regression tests. 1003 tests, tsc clean, build green. Commit 73f1bce |
