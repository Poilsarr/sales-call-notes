...
- **Meta-in-body (P1, from LIGHTHOUSE-CI arc, 2026-08-14)** — all metadata (description/og/robots, ~19 metas) lands at the TOP OF `<body>` instead of `<head>` → Lighthouse `meta-description` = 0, SEO capped at ~92. Mechanism (agent-verified): `@clerk/nextjs@5.7.6` server `ClerkProvider` calls `headers()` (`node_modules/@clerk/nextjs/dist/esm/app-router/server/ClerkProvider.js:11`) → every route renders dynamic → root `src/app/loading.tsx` fallback-first streaming → Next injects metadata into the streamed body. Fix options for a future structural arc: upgrade to Clerk 7.7.5+ (server `ClerkProvider` accepts `dynamic` prop, no `headers()` call — verified in the 7.7.5 dist) or switch layout to client `ClerkProvider` with `<head>` kept manual. Meanwhile: Lighthouse CI keeps SEO as warn (0.90); metadata is correctly emitted, just misplaced — docs/social behavior unaffected. See `.lighthouserc.js` comment + `LIGHTHOUSE-CI-PRD.md`.
