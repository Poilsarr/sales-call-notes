...
| --- | ---------- | ------------------------------------------------------------------ | --------------------------------------- |
| —   | 2026-08-23 | fix(security): close 3 high vulns + guard debug oracles — audit 3→0 | Wave5 overrides via `package.json` (no `next@16` major): `postcss@8.5.14→8.5.26` (GHSA-qx2v etc) + `sharp@0.34.5→0.35.0` (`libvips 1.2.4→1.3.0`, GHSA-f88m) — root + `next` deduped, `npm audit` **0** (`26→0`), `next@15.5.23` retained. `src/app/api/debug/transcription-test/route.ts:5` + `src/app/api/billing/debug/route.ts:3` now `404` in `VERCEL_ENV=production` (closes authenticated info disclosure: `openaiKeySet`/`stack`/`Paddle` state). `.env` `600` (was `644`). Commit `23736ee` (Hermes Agent), `CI #555` `2m15s` green, `136/1139` green (2 stale proof freshness excluded). Plan `DEPS-UPGRADE-PLAN.md` Wave5 shipped. |
| —   | 2026-08-22 | deps(wave1-4): vite/esbuild/nanoid/fast-uri/brace-expansion + undici/form-data/js-cookie + clerk 1.34 + lhci remove — audit 26→3 | Arc C Wave1 (f9a925f) `vite@8.0.16`, `esbuild@0.28.2`, `nanoid@3.3.18` + `fast-uri@3.1.5` via `@sentry/nextjs@10.70.0` (`@opentelemetry` moderate fixed), `brace-expansion@5.0.9` — `overrides` — `26→18` (15 high). Wave2 (c23e593) `@vercel/blob@2.8.0` (`undici@6.28/7.29`), `form-data@4.0.6`, `js-cookie@3.0.8` — `18→13` (10 high). Wave3 (4f566dc) `@clerk/backend@1.14.1→1.34.0` unify `shared@2.9.2→3.47.8` — `13→13` (no audit advisory, dedupe). Wave4 (0bfe585) `npm remove @lhci/cli` (abandoned `0.15.1`, `extract-zip@2.0.1` still GHSA) — `13→3` (only `next@15.5.23` + `postcss@8.5.14` + `sharp@0.34.5` remain for Wave5). Plan `docs/roadmap/execution/plans/DEPS-UPGRADE-PLAN.md`, gates `136/1139`, `tsc` clean, `build` 105kB, `next` retained. |
| —   | 2026-08-22 | fix(ci): lighthouse workflow YAML — quote run string (colon broke parser) | Workflow `.github/workflows/lighthouse.yml:55` `run: echo "lighthouse: ..."` colon broke YAML (`mapping values not allowed`), `python yaml.safe_load` failed, GitHub reported `X .github/workflows/lighthouse.yml · push · failure`. Fix: single-quoted `echo 'lighthouse removed...'`, `YAML OK`, `CI` + `React Doctor` green, `Lighthouse CI` no longer runs on push (on: `pull_request` + `workflow_dispatch` only). Commit `fdc81e4`. |
| —   | 2026-08-21 | fix(auth): CLERK-LOGIN-FIX — sign-in wall + P2002 + clerkId + DSN gate | 4 commits, plan `docs/roadmap/execution/plans/CLERK-LOGIN-FIX-PLAN.md` (SHIPPED 2026-08-21). E1 (86455de) `src/app/sign-in/[[...sign-in]]/page.tsx` + `src/app/sign-up/[[...sign-up]]/page.tsx`: `routing="path" path="/sign-in"` for catch-all `[[...sign-in]]` + factor-one/sso-callback/OAuth hash (Safari ITP), `useAuth` `isSignedIn` guard + `AlreadySignedInCard` with `SignOutButton` (clears `__session`) + Go to app CTA, `fallbackRedirectUrl="/app"`. E2 (a15bdcf) `src/lib/get-user.ts`: `update:{}` → `update:{email,name}` sync, `try/catch P2002` on `email @unique` → `findUnique where email` + Sentry, `src/app/app/layout.tsx:14` `where:{id:userId}` → `where:{clerkId:userId}` (cuid vs `user_2xxx` always-null), `src/app/api/personalization/route.ts:43,75` same, new `src/test/get-user.test.ts` 8 tests. E3 (cd83be4) `src/middleware.ts:91` `NEXT_PUBLIC_SENTRY_DSN` → `NEXT_PUBLIC_SENTRY_DSN \|\| SENTRY_DSN` (server silent), Clerk key-pair comment. Gate 134/1133 green, `ƒ /sign-in` 3.51kB/138kB, shared 105kB. |
| —   | 2026-08-21 | fix(ui): dashboard billing fallback + toaster + features lazy polish | Audit fix post-BUNDLE (4.19). Dashboard keeps `billing.planName` in `billing?` branch + Free defaults in else (was `Unable` flash); `toaster-host.tsx` named export + tradeoff doc; `features-page-client.tsx:11` single `dynamic` (was 2 waterfalls duplicating `gsap`), `features-animations.tsx:118` `opacity-0→gsap.from` (3G pop-in); `global-error.tsx:14` second `cancelled` guard + try/catch. Commit `d441a6c`, part of audit wave `81765d1..d441a6c`, 133/1125 green. |
| —   | 2026-08-21 | fix(trust): security copy + sentry DSN + chat model + hygiene | Audit fix 4.19: `src/app/security/page.tsx:253` `7-day grace` → immediate purge (code is inline), `src/app/privacy/page.tsx:50` add `Clerk` + `Paddle` to sub-processors (was 6, now 8); `src/lib/safe-error.ts:29` + `src/lib/rate-limit.ts:90` `NEXT_PUBLIC_SENTRY_DSN \|\| SENTRY_DSN` (server silent); `src/app/api/chat/route.ts:78` `gpt-4o→gpt-4o-mini` (~10×); `.gitignore` `graphify-out/` + `scripts/.fixtures/` + `scripts/test-calls/`. Commit `a1abded`. |
| —   | 2026-08-21 | fix(api): billing plan guard + presigned size gate + orphan cleanup + 500MB ceiling | Audit fix 4.19: `src/app/api/billing/route.ts:15` `cfg=PLANS[plan]??free` (was `planName` guarded but `limit/minuteLimit/features` 500 on unknown `foobar`); `src/app/api/analyze/route.ts:137` byte-count gate after `fetch(blobUrl)` (client `fileSize:1` bypass) + orphan `del` on 413/validation fail (was `uploadedOwnBlob=false` leak) + `try{del} catch{}` for mock safety; `src/services/validation/file-validation.ts:25` `100→500MB` (Pro 200/Business 500 blocked). Tests: `billing-plan-name.test.tsx` unknown plan cases, `upload-privacy.test.ts` JSON orphan + size gate. Commit `63ca3e1`, 133/1125. |
| —   | 2026-08-21 | fix(security): detail-route audioUrl leak + blob trust fail-closed | Audit fix 4.19: `GET /api/calls/[id]:29` + `GET /api/history/[id]:19` `include` leaked private `audioUrl` to team viewers + 300s cache; now explicit `select` without `audioUrl` (playback via `/api/calls/[id]/audio` proxy); `src/lib/blob-url.ts:57` fail-open when `BLOB_STORE_ID` unset → any `*.blob.vercel-storage.com` → exfiltrates `BLOB_READ_WRITE_TOKEN`, now `return false` (fail-closed). `blob-url-guard.test.ts` now expects `false` for attacker store. Commit `81765d1`. |
...
| —   | 2026-08-21 | feat(intelligence): V2 — your rivals + living visuals (watchlist + radar/heatmap/river/treemap) | V2a (0c92875): User/Team.companyName + TrackedCompetitor (XOR teamId/userId, partial unique on normalizedName, CHECK XOR) + CompetitorMention denorm (userId/teamId/normalizedCompetitor/isWatchlistHit/matchedEntryId) additive migration 20260821130000; libs src/lib/competitor-watchlist.ts (COMPETITOR_NAME_RE, control-char reject, JSON.stringify fencing B-02) + src/lib/company-suggest.ts (domain chip); API GET/PUT /api/company + GET/POST /api/competitors + PATCH/DELETE /api/competitors/[id] with IDOR 404 (B-01), allowlist, P2002, per-clerk limit, RBAC MEMBER/ADMIN (H-03), free gate H-04, auditLog; GET /api/competitive-intelligence mode=watchlist|all, sanitize %_\\, normalized exact, no userId leak, explicitRange fix (allForTrend), meta; POST /api/analyze injects watchlist prompt after vocabulary, marks hits. V2b: new src/components/gauge-viz/{threat-radar,battle-heatmap,momentum-river,share-treemap} + src/lib/viz/math.ts — pure SVG + framer-motion zero deps, Threat Radar (r=velocity theta=share), Battle Heatmap (competitor×sentiment×recency), Momentum River (stacked bezier), Treemap (share + threat border); page adds mode toggle, header meta, truthful empty copy, Company & Competitors card in settings. 136 files 1139 tests, tsc clean, build green (intelligence 11.6kB), migration additive. Plan docs/roadmap/execution/plans/INTELLIGENCE-V2-PLAN.md, security amendments B-01..H-04, 3-architect swarm + 2-reviewer verification. |
