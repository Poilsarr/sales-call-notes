...
| Bundle drift: 8 routes 7–15% over June budgets (freshest `/features` 279 kB vs 260, `/dashboard` 237 vs 210, `/settings` 250 vs 215, `/billing` 234 vs 220, `/pricing` 229 vs 210, `/` 222 vs 220, `/demo` 191 vs 180, `/onboarding` 190 vs 175). Proof stale since ed323a3 (June); gate only guards committed snapshot → CI green on stale numbers. Arc B of ARCS-BACKLOG: re-baseline + optimize (clerk split, route-level splitting, drop client PLANS chunk from /billing /dashboard /settings) | MED | Perf gate honesty + real gains |
| Dep upgrades: 26 vulns / 19 high (arc C — deliberate gated pass, no blind --force) | HIGH | Audit blocker, pre-launch |
| Settings toast copy on account delete is stale ("scheduled… sign in within 7 days to cancel") — hard delete is now inline, not queued | LOW | Honesty (W-B follow-up, needs UI PR) |
