...
export default function CallSearch() {
  ...
  const lastQuery = state.kind === "loading" || state.kind === "error" ? state.query : "";
    ...
    <div>
      ...
      >
        ...
        <button
          ...
          className="px-4 py-2.5 bg-linear-indigo rounded-xl text-xs font-semibold hover:bg-linear-indigo/80 transition disabled:opacity-50 inline-flex items-center gap-2"
        >
          {busy ? (
      ...
      <div className="mt-3" role="status" aria-live="polite">
        {state.kind === "success" && state.results.length === 0 && (
          <div className="p-4 rounded-xl bg-linear-black border border-linear-secondary border-dashed text-center">
            <p className="text-sm text-white/40">No calls matched “{state.query}”.</p>
            <p className="text-xs text-white/30 mt-1">Try a phrase like “objections”, “budget”, or “next steps”.</p>
          </div>
        )}
            ...
            <div className="space-y-2">
              {state.results.map((call) => (
                <Link
                  ...
                  className="flex items-start gap-3 p-3 rounded-xl bg-linear-black border border-linear-secondary hover:border-linear-indigo/40 transition group"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline justify-between gap-3">
                      ...
                      <span className="text-[11px] text-linear-indigo shrink-0">
        ...
        {state.kind === "error" && (
          <div className="p-4 rounded-xl bg-linear-black border border-red-500/20">
            <p className="text-sm text-red-400/80 mb-3">Couldn't search your calls.</p>
            <button
              type="button"
              onClick={() => void runSearch(state.query)}
              disabled={busy}
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-xs font-medium text-red-400 hover:bg-red-500/20 transition disabled:opacity-50"
            >
              {state.pending ? (
